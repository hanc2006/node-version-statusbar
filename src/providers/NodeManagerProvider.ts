...
export class NodeVersionProvider implements vscode.Disposable {
  ...
  private workspaceNodeRequirement: string | null = null;
  ...
  async refresh(): Promise<void> {
    try {
      ...
      await this.refreshWorkspaceNodeRequirement();
  ...
  private async refreshWorkspaceNodeRequirement(): Promise<void> {
    const workspaceFolder = vscode.workspace.workspaceFolders?.[0];
    if (!workspaceFolder) {
      this.workspaceNodeRequirement = null;
      return;
    }
    ...
    try {
      const contents = await vscode.workspace.fs.readFile(packageUri);
      const packageJson = JSON.parse(Buffer.from(contents).toString("utf8"));
      ...
      this.workspaceNodeRequirement = nodeRequirement && nodeRequirement.length > 0
        ? nodeRequirement
        : null;
    } catch (error) {
      console.warn(
        "Unable to read workspace package.json for Node.js engine requirement:",
        error,
      );
      this.workspaceNodeRequirement = null;
    }
  ...
  private updateStatusBar(version: string | null, errorMessage?: string) {
    if (!version && !errorMessage) {
      ...
      if (this.workspaceNodeRequirement) {
        const cleaned = semver.coerce(version ?? "");
        if (cleaned) {
          if (!semver.satisfies(cleaned, this.workspaceNodeRequirement, { includePrerelease: true })) {
            this.statusBarItem.backgroundColor = new vscode.ThemeColor(
              "statusBarItem.warningBackground",
            );
            tooltip += `\nWorkspace requires Node ${this.workspaceNodeRequirement}. Update your runtime to meet this requirement.`;
          } else {
            this.statusBarItem.backgroundColor = new vscode.ThemeColor(
              "statusBarItem.prominentBackground",
            );
            tooltip += `\nRequirement ${this.workspaceNodeRequirement} satisfied.`;
          }
        } else {
          tooltip += `\nWorkspace requires Node ${this.workspaceNodeRequirement}.`;
        }
      }
